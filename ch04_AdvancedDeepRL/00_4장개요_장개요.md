...
4장을 다 읽고 나면 다음 항목을 스스로 설명할 수 있어야 한다.

| 절 | 핵심 |
|---|---|
| 4.1 Deep RL Project 준비 | Reinforcement Learning Project 를 위한 준비 |
| 4.2 DDQN(Double DQN) | DQN 의 Overestimation Bias 감소 |
| 4.3 TD3(Twin Delayed DDPG) | DDPG + Clipped Double Q + Delayed policy update + Target Policy Smoothing |
| 4.4 Asynchronous Advantage Actor-Critic(A3C) | A2C + Asynchronous method. correlation 문제 해결, 학습 속도 향상 |
| 4.5 Proximal Policy Optimization(PPO) | 정책 함수 $\pi_\theta$ 를 천천히 바꾸어 Actor-Critic 불안정성 극복 |
| 4.6 Soft Actor-Critic(SAC) | MaxEnt RL(Entropy-Regularized RL) + Q-function. Stochastic policy optimization 과 DDPG-style approaches 의 중간에 위치 |
